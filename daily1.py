...
# list type of object with data
list1 = [1, 2, 3, 4, 5, 'a', 'JAI', "Arora", True]
list1  # print when through spyder
type(list1)  # type of object
print(list1)  # print when running complete file
list1
sorted([3456, 9876, 22228, 3330303, 45466474, 888, 3433, 4745])
list2 = ['f', 'c', 'd', 'e', 'f', 'a', 'j', 'k', 'z', 'f', 'm']
list2
list2.count('f')
list2.count('m')
len(list2)
...
# print?  #help
for i in list2:
    print(i, 'DU', sep='-', end='xxx')
#a-DU-xxx ; i+sep+end
for i in list1:
    print(i, 'HHHE', sep=' X ', end='\t')
...
x = np.arange(start=0, stop=1000000, step=1)
len(x)
x[0:100]
x[0:50:10]
np
...
df1 = pd.DataFrame({'rollno': [1, 2, 3, 4, 5], 'name': ["Dhiraj", "Kounal", "Akhil", "Pooja", 'Jai'], 'marks': [
                   40, 50, 60.5, 70, 99], 'gender': ['M', 'M', 'M', 'F', 'M']})
df1
type(df1)
...
import pandas as pd
np.sort(dataset)
np.quantile(a=dataset, q=[0,.25,.50,.75,1])
...
df1.groupby('gender').aggregate(
    {'marks': [np.mean, 'max', 'min', 'std', 'count']})
